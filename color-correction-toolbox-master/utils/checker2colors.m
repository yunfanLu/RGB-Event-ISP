...
% vertex_pts：       4*2 coordinates (brown-first order) to replace interactive operation.
%
% OUTPUTS:
...
if isempty(param.roi)
    switch lower(param.mode)
        ...
        case 'auto'
            disp('use given coordinate')
            % to be done in the future
            vertex_pts = param.vertex_pts
        otherwise
...
parser.addParameter('vertex_pts', [], @(x)validateattributes(x, {'numeric'}, {'positive'}));
