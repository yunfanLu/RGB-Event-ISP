...
blc     =  min(img_blc(:));
img_fpn=img_blc - blc;
imwrite(img_fpn,'rawdata/img_fpn.jpg');
...
figure();
imshow(img_rgb);
imwrite(img_rgb,'rawdata/img_rgb_demosaicd.jpg')

%interactive get wb and ccm
[colors, coord] = checker2colors(img_rgb, [4, 6], 'mode', 'click', 'show', false);
...
figure();
imshow(img_wb);
imwrite(img_wb,'rawdata/img_wb.jpg')
...
figure();
imshow(img_denoise);
imwrite(img_denoise,'rawdata/img_denoise.jpg')
...
figure();
imshow(img_srgb);
imwrite(img_srgb,'rawdata/img_srgb.jpg');
